...
class SeModule(nn.Module):
    def __init__(self, in_size, reduction=4):
        ...
        self.se = nn.Sequential(
            ...
            nn.BatchNorm2d(in_size // reduction, eps=1e-03),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_size // reduction, in_size, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(in_size, eps=1e-03),
            hsigmoid()
        )
...
class Block(nn.Module):
    ...
    def __init__(self, kernel_size, in_size, expand_size, out_size, nolinear, semodule, stride, dilation):
        ...
        self.bn1 = nn.BatchNorm2d(expand_size, eps=1e-03)
        self.nolinear1 = nolinear
        self.conv2 = nn.Conv2d(expand_size, expand_size, kernel_size=kernel_size, stride=stride, padding=(kernel_size//2)*dilation, groups=expand_size, dilation=dilation, bias=False)
        self.bn2 = nn.BatchNorm2d(expand_size, eps=1e-03)
        self.nolinear2 = nolinear
        self.conv3 = nn.Conv2d(expand_size, out_size, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(out_size, eps=1e-03)
...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(3, 16, 72, 24, nn.ReLU(inplace=True), None, 1, 2),
            Block(3, 24, 88, 24, nn.ReLU(inplace=True), None, 1, 5),
            Block(5, 24, 96, 40, hswish(), SeModule(40), 2, 1),
            Block(5, 40, 240, 40, hswish(), SeModule(40), 1, 2),
            Block(5, 40, 240, 40, hswish(), SeModule(40), 1, 5),
            Block(5, 40, 120, 48, hswish(), SeModule(48), 1, 9),
            Block(5, 48, 144, 48, hswish(), SeModule(48), 1, 2),
            Block(5, 48, 288, 96, hswish(), SeModule(96), 1, 5),
            Block(5, 96, 576, 96, hswish(), SeModule(96), 1, 9),
            Block(5, 96, 576, 96, hswish(), SeModule(96), 1, 17),
        )
